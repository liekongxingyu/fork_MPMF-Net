...
if __name__ == '__main__':
    ...
    datasetTrain = MyTrainDataSet(
        inputPathTrain, targetPathTrain, patch_size=PATCH_SIZE_TRAIN)
    ...
    datasetValue = MyValueDataSet(
        inputPathVal, targetPathVal, patch_size=PATCH_SIZE_VAL)
    ...
    if os.path.exists(opt.MODEL_RESUME_PATH):
        if opt.CUDA_USE:
            ...
            myNet.load_state_dict(torch.load(
                opt.MODEL_RESUME_PATH, map_location=torch.device('cpu')))
    ...
    for epoch in range(EPOCH):
        ...
        for index, (x, y) in enumerate(iters, 0):
            ...
            with torch.cuda.amp.autocast(True):
                ...
                loss_inr = F.l1_loss(
                    restored[1], F.interpolate(target, scale_factor=0.25))
            ...
            iters.set_description(
                'Training !!!  Epoch %d / %d' % (epoch+1, EPOCH))
        ...
        if epoch % 1 == 0:
            ...
            for index, (x, y) in enumerate(valueLoader, 0):
                input_, target_value = (
                    x.cuda(), y.cuda()) if opt.CUDA_USE else (x, y)
                with torch.no_grad():
